...
describe Chronosify::Calendar do
  let(:calendar) { Chronosify::Calendar.new(ViewContext.new) }

  it 'renders a partial with the same name as the class' do
    expect(calendar.send(:partial_name)).to eq("chronosify/calendar")
  end
  ...
  context 'event sorting attribute' do
    ...
    it 'allows you to override the default attribute' do
      expect(Chronosify::Calendar.new(ViewContext.new, attribute: :starts_at).send(:attribute)).to eq(:starts_at)
    end

    it "set a default when `partial` option isn't present" do
      expect(Chronosify::Calendar.new(ViewContext.new).send(:partial_name)).to eq('chronosify/calendar')
      expect(Chronosify::MonthCalendar.new(ViewContext.new).send(:partial_name)).to eq('chronosify/month_calendar')
      expect(Chronosify::WeekCalendar.new(ViewContext.new).send(:partial_name)).to eq('chronosify/week_calendar')
    end

    it 'allows to override the default partial' do
      expect(Chronosify::Calendar.new(ViewContext.new, partial: 'chronosify/custom_calendar').send(:partial_name)).to eq('chronosify/custom_calendar')
    end
  end
  ...
  describe "#sorted_events" do
    it 'converts an array of events to a hash sorted by days' do
      ...
      calendar = Chronosify::Calendar.new(ViewContext.new, events: events)
    ...
    it 'handles events without a start time' do
      ...
      calendar = Chronosify::Calendar.new(ViewContext.new, events: [event])
  ...
  describe "#start_date" do
    it "defaults to today's date" do
      ...
      calendar = Chronosify::Calendar.new(view_context)
    ...
    it "uses the optional argument's start_date to override view_context's start_date" do
      view_context = ViewContext.new(Date.yesterday)
      calendar = Chronosify::Calendar.new(view_context, start_date: Date.tomorrow)
