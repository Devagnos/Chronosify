...
Chronosify is designed to do one thing really really well: render a
...
generators for chronosify:

```bash
$ rails g chronosify:views
```

This will generate a folder in app/views called chronosify that you
...
chronosify comes with a handful of useful classes for each day in
...
needs, but chronosify makes it easy to create completely custom
calendars (like maybe you only want business weeks).

If you'd like to make a completely custom calendar, you can create a new
class that inherits from `Chronosify::Calendar`. The name you give
...
`app/views/chronosify/_business_week_calendar.html.erb` partial.
...
[devagnos.com](devagnos.com)

[@hatchvenom](https://twitter.com/hatchvenom)
